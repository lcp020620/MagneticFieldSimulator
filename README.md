...
You must install the appropriate versions of CUDA and CuPy for the version I mentioned above. CUDA is the core of this program, so NVIDIA graphic card is required. Since I haven't tested it in a virtual environment for users without a graphics card or with CPU built-in graphic card, I can't guarantee proper program execution.
You'll also need to install Flask and Numpy, but their versions are relatively insignificant. Your web browser must support JavaScript's Three.js.
